...
function NavOptionalBar({ data, onToast, inlinePadding }) {
  ...
  let innerMessageCount =
    data.messageCount > 99
      ? "99+명이 작성했어요 !"
      : `${data.messageCount}명이 작성했어요 !`;
  ...
  return (
    <S.EntireWrapper>
      <S.UserBox inlinePadding={inlinePadding}>{`To. ${data.name}`}</S.UserBox>
                  ...
                  <S.EmojiListModalInnerWrapper>
                    {emojiListChange
                      ? new Array(
                          Math.min(
                            4,
                            Math.max(Object.keys(emojiData).length - 3, 0)
                          )
                        )
                          .fill(0)
                          .map((x, index) => {
                  ...
                  <S.EmojiListModalInnerWrapper>
                    {emojiListChange
                      ? new Array(
                          Math.min(
                            4,
                            Math.max(Object.keys(emojiData).length - 7, 0)
                          )
                        )
                          .fill(0)
                          .map((x, index) => {
                      ...
                      : new Array(
                          Math.min(
                            3,
                            Math.max(Object.keys(emojiData).length - 6, 0)
                          )
                        )
                          .fill(0)
                          .map((x, index) => {
